...
string handleDeleteByTypeAndDate(const string& directory, const string& fileType, time_t beforeTimestamp) {
    stringstream json;
    json << "{\"success\":true,\"deletedFiles\":[";
            ...
            try {
                if (entry.is_regular_file()) {
                    ...
                    if (typeMatches) {
                        ...
                        time_t fileTime = std::chrono::system_clock::to_time_t(sys_time);
    ...
    json << "],\"count\":" << deletedCount << ",\"message\":\"Deleted " << deletedCount << " files\"}";
    return json.str();
}
